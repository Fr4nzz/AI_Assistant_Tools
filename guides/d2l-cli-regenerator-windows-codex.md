# D2L/Brightspace CLI transferable setup for Codex Desktop on Windows, Linux, and WSL

This Markdown file contains setup notes plus the live `d2l.py` script and Codex skill so another Codex Desktop user can recreate the D2L/Brightspace integration without a zip.

## What this does

This CLI gives read-only access to USFQ D2L/Brightspace courses, assignments, deadlines, grades, announcements, course content, calendar events, feedback, unread items, and course file downloads.

It uses the shared Outlook/Office Chromium profile by default plus Chrome DevTools Protocol (CDP) to extract authenticated D2L cookies. This lets one Microsoft login bootstrap Outlook, OneDrive, and D2L. It does not store passwords and does not print cookies.

## Platform behavior

- D2L base URL: `https://miusfv.usfq.edu.ec`.
- Default CDP port: `18801`.
- Avoid CDP port `18800` on Franz's setup because it may be bound by a Franz-PC SSH tunnel.
- D2L cache/lock path on Windows: `%LOCALAPPDATA%\d2l-cli`.
- D2L cache/lock path on Linux/WSL: `~/.local/share/d2l-cli`.
- Browser profile by default: the shared Outlook profile (`%LOCALAPPDATA%\outlook-cli\browser-data` on Windows, `~/.local/share/outlook-cli/browser-data` on Linux/WSL). Override with `D2L_BROWSER_DATA_DIR` only if an isolated D2L profile is needed.
- Browser startup lock: `browser.lock` inside the data/profile path, so parallel chats do not start duplicate helper browsers.
- Windows launcher: `d2l.cmd`.
- Linux/WSL launcher: install `d2l.py` as executable `d2l`.

## Safety model

- Read-only only.
- Do not submit assignments, delete content, edit content, send messages, or change grades.
- Do not repeatedly retry failed logins. If login/session fails, ask the user to run `d2l login` and sign in manually.
- Do not print cookies or tokens.
- Do not expose private course/classmate information unnecessarily.
...
```powershell
mkdir D:\VSCodeProjs2\_codex_d2l_setup
cd D:\VSCodeProjs2\_codex_d2l_setup
```

Linux/WSL example:

```bash
mkdir -p ~/d2l-cli-setup ~/.local/bin
cd ~/d2l-cli-setup
```

Create `d2l.py` from the file block below. On Windows also create `d2l.cmd`.
...
Windows: run from the setup folder with `d2l.cmd`, add the setup folder to PATH, or create a shim at `%USERPROFILE%\.local\bin\d2l.cmd` because Codex Desktop normally includes `.local\bin` in its process PATH.

```bat
@echo off
call D:\VSCodeProjs2\_codex_d2l_setup\d2l.cmd %*
```

Linux/WSL:

```bash
cp d2l.py ~/.local/bin/d2l
chmod +x ~/.local/bin/d2l
d2l --help
```
...
A visible browser opens using the shared Outlook/Office helper profile. If you already logged into Outlook/OneDrive with this profile, D2L may only need a quick SSO redirect. Otherwise sign in to D2L/Microsoft, choose **Stay signed in** if prompted, wait until D2L loads, then close that browser before running headless CLI commands.

The headless CLI can then reuse the session cookies. If Microsoft/D2L expires the session later, run `d2l login` again.

## Verify headless use

```powershell
.\d2l.cmd --json classes
.\d2l.cmd deadlines
.\d2l.cmd assignments
```
...
If the CLI says auto-login failed or no cookies were found, stop and run `d2l login`; do not repeatedly retry failed login commands.

## Install as a global Codex Desktop skill

The skill can stay generic with `bins: ["d2l"]` if `d2l` is on PATH.

```powershell
$skillDir = Join-Path $HOME ".codex\skills\d2l"
New-Item -ItemType Directory -Force -Path $skillDir | Out-Null
Copy-Item -LiteralPath ".\d2l_SKILL.md" -Destination (Join-Path $skillDir "SKILL.md") -Force
```

```bash
mkdir -p ~/.codex/skills/d2l
cp d2l_SKILL.md ~/.codex/skills/d2l/SKILL.md
```
...
- Start with `d2l --json classes` to get course IDs.
- For pending work, use `d2l --json deadlines`, `d2l --json assignments`, and `d2l --json calendar --days 14`.
- For course-specific context, use `--course COURSE_ID`.
- For assignment details, use `d2l assignment ASSIGNMENT_ID`.
- For grades/comments, use `d2l feedback ASSIGNMENT_ID`.
- For course materials, use `d2l content --course COURSE_ID`; download only files needed for the task.

## Removal

Delete the command files and skill folder:

```powershell
Remove-Item -Recurse -Force D:\VSCodeProjs2\_codex_d2l_setup
Remove-Item -Recurse -Force $HOME\.codex\skills\d2l
Remove-Item -Force $HOME\.local\bin\d2l.cmd
```

```bash
rm -rf ~/d2l-cli-setup ~/.codex/skills/d2l ~/.local/bin/d2l
```

To remove saved browser session data, also delete `%LOCALAPPDATA%\d2l-cli` on Windows or `~/.local/share/d2l-cli` on Linux/WSL.
