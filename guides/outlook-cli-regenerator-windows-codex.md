# Outlook CLI transferable setup for Codex Desktop on Windows, Linux, and WSL

This Markdown file contains setup notes plus the live `outlook.py` script and Codex skill so another Codex Desktop user can recreate the setup without a zip.

## What this does

This CLI gives read-only access to Outlook/Microsoft 365 mail and calendar for accounts where third-party Azure apps require admin approval. Instead of registering a third-party Graph app, it uses a persistent Chromium profile for Outlook Web (`https://outlook.cloud.microsoft/mail/`), extracts the official OWA MSAL access token from browser localStorage, and calls the Outlook REST API v2.0.

Microsoft Graph is not used for Outlook mail/calendar in this setup because the Graph token available from Outlook Web may have file/profile scopes but not Graph `Mail.Read` or `Calendars.Read` scopes. The Outlook-audience token does have the mailbox/calendar scopes needed by Outlook REST.

## Platform behavior

- Windows data/profile path: `%LOCALAPPDATA%\outlook-cli`.
- Linux/WSL data/profile path: `~/.local/share/outlook-cli`.
- Token cache: `token.json` inside the data/profile path.
- Token refresh lock: `token.lock` inside the data/profile path, so parallel chats do not open the same Chromium profile at the same time.
- Browser profile: `browser-data` inside the data/profile path.
- Windows launcher: `outlook.cmd`.
- Linux/WSL launcher: install `outlook.py` as executable `outlook`.
- Windows encoding: the script forces UTF-8 stdout/stderr, so JSON and Spanish/accented output should work without extra encoding fixes.

## Create the files

Create a setup folder, then create `outlook.py` from the file block below. On Windows also create `outlook.cmd`.

Windows example:

```powershell
mkdir D:\VSCodeProjs2\_codex_outlook_setup
cd D:\VSCodeProjs2\_codex_outlook_setup
```

Linux/WSL example:

```bash
mkdir -p ~/outlook-cli-setup ~/.local/bin
cd ~/outlook-cli-setup
```
...
Windows: run from the setup folder with `outlook.cmd`, add the setup folder to PATH, or create a shim at `%USERPROFILE%\.local\bin\outlook.cmd` because Codex Desktop normally includes `.local\bin` in its process PATH.

```bat
@echo off
call D:\VSCodeProjs2\_codex_outlook_setup\outlook.cmd %*
```

Linux/WSL: install the script as `outlook` on PATH.

```bash
cp outlook.py ~/.local/bin/outlook
chmod +x ~/.local/bin/outlook
outlook --help
```

## First login/bootstrap with a visible browser

The first login must use a visible/headed browser with the same persistent profile that the headless CLI will later use. Sign in to Outlook, choose **Stay signed in**, wait until Outlook mail loads, then close that browser window.

Windows PowerShell:

```powershell
$chrome = python -c "import outlook; print(outlook._find_chromium())"
$data = python -c "import outlook; print(outlook.BROWSER_DATA_DIR)"
New-Item -ItemType Directory -Force -Path $data | Out-Null
Start-Process -FilePath $chrome -ArgumentList @("--user-data-dir=$data", "https://outlook.cloud.microsoft/mail/")
```

Linux/WSL with GUI/WSLg:

```bash
chrome=$(python3 -c "import outlook; print(outlook._find_chromium())")
data=$(python3 -c "import outlook; print(outlook.BROWSER_DATA_DIR)")
mkdir -p "$data"
"$chrome" --user-data-dir="$data" https://outlook.cloud.microsoft/mail/
```

Important: close the visible browser before running the headless CLI. Chromium cannot open the same persistent profile twice. The CLI has a token-refresh lock for parallel chats, but that lock only coordinates CLI refreshes; it cannot share the profile with a manually opened browser window.

After the first successful token refresh, normal commands should run headless without opening a visible browser. A visible browser is only needed again if Microsoft expires the saved session or requires re-authentication.
...
The skill can stay generic with `bins: ["outlook"]` if `outlook` is on PATH. That is the cleanest setup for both Linux and Windows once the shim or PATH entry exists.

```powershell
$skillDir = Join-Path $HOME ".codex\skills\outlook"
New-Item -ItemType Directory -Force -Path $skillDir | Out-Null
Copy-Item -LiteralPath ".\outlook_SKILL.md" -Destination (Join-Path $skillDir "SKILL.md") -Force
```

```bash
mkdir -p ~/.codex/skills/outlook
cp outlook_SKILL.md ~/.codex/skills/outlook/SKILL.md
```

Optional global fallback at `~/.codex/AGENTS.md`:

```markdown
For Outlook/Microsoft 365 email or calendar requests, use the global `outlook` skill and local read-only `outlook` CLI.
```
...
## Search and performance tips

- Do not start with `outlook --help` unless a command fails.
- For topic context, run one broad search, 2-3 targeted variants, dedupe by conversation/subject/sender/date, then read representative messages.
- Use `outlook read MSG_ID`; attachment names are usually useful context and are listed automatically.
- Use `outlook thread MSG_ID -n 20` only when prior replies are needed. Thread fetches are slower by design.
- Use `outlook search --since 7d -n 80` for normal date-window scans. The CLI uses OData for structured date-only searches.
- Avoid Gmail-style operators like `after:`/`before:` and avoid combining `$search` with `$filter` or `$orderby`.
- Full-text topic search uses Microsoft server-side KQL and can take longer than date scans.
- Parallel commands are fine. If the token is expired, one process refreshes it while the other waits for `token.lock`.

## Security notes

- Do not hardcode passwords.
- Do not share `token.json`.
- Do not share the `browser-data` profile; it contains credentials/session state.
- The exposed CLI commands are read-only.
...
```bat
@echo off
python "%~dp0outlook.py" %*
```

## File: `outlook_SKILL.md`
